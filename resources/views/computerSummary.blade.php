...
<div class="shadow-lg mt-4 p-5 bg-secondary text-white rounded" style="margin: 60px;position: absolute;width:90%">
    <div style="float:left;width:50%;">
        <h5>You have built a computer with the following specs:</h5>
        <h1>{{ $computer['name'] }}</h1>
        <h3>Chassis</h3>
        ...
        <form action="{{ route('computer.destroy', $computer->id) }}" method="POST">
            ...
            <button type="submit" onclick="return confirm('Are you sure to delete {{ $computer['name'] }}?')" class="btn btn-danger">Delete</button>
        </form>
    </div>
